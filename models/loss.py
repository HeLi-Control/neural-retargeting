...
def calculate_all_loss(data_list, target_list, ee_criterion, vec_criterion, col_criterion,
    ...
    if reg_losses is None:
        reg_losses = []
    if fin_losses is None:
        fin_losses = []
    if ori_losses is None:
        ori_losses = []
    if lim_losses is None:
        lim_losses = []
    if col_losses is None:
        col_losses = []
    if vec_losses is None:
        vec_losses = []
    if ee_losses is None:
        ee_losses = []
    ...
    # joint limit loss
    if lim_criterion:
        lim_loss = (calculate_lim_loss(target_list, target_ang, lim_criterion) *
                    float(loss_gain[3]))
    ...
    if fin_criterion:
        fin_loss = calculate_fin_loss(data_list, target_list, l_hand_pos, r_hand_pos,
                                      fin_criterion) * float(loss_gain[5])
...
if __name__ == '__main__':
    ...
    print(col_loss.capsule_capsule_dist_square(fake_capsule_p0, fake_capsule_p1,
                                               fake_capsule_q0, fake_capsule_q1,
                                               1, 2))
